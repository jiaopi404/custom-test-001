          ...
          >
            <template slot-scope="{ row }">
              <template v-if="currentRowId !== row.id">
                <el-button type="text" @click="editProSpecify(row)">编辑预算细化</el-button>
                <el-button type="text" @click="showEconomicSubject(row)">查看经济科目</el-button>
                <el-button type="text" @click="delProject(row)">删除</el-button>
              </template>
              <template v-else>
  ...
  computed: {
    ...
    totalAmount () {
      return this.tableData.reduce((prev, item) => {
        prev += item.id === -999 ? 0 : item.budgetAmount
        return prev
      }, 0)
    }
  },
