...
(function (DocsAPI, window, document, undefined) {
  ...
  */
  ...
  DocsAPI.DocEditor = function (placeholderId, config) {
    ...
    _config.editorConfig.canRequestCompareFile =
      _config.events && !!_config.events.onRequestCompareFile;
    _config.editorConfig.canRequestSharingSettings =
      _config.events && !!_config.events.onRequestSharingSettings;
    _config.editorConfig.canRequestCreateNew =
      _config.events && !!_config.events.onRequestCreateNew;
    ...
    var _checkConfigParams = function () {
      if (_config.document) {
        ...
        if (
          ...
          var type =
            /^(?:(xls|xlsx|ods|csv|xlst|xlsy|gsheet|xlsm|xlt|xltm|xltx|fods|ots)|(pps|ppsx|ppt|pptx|odp|pptt|ppty|gslides|pot|potm|potx|ppsm|pptm|fodp|otp)|(doc|docx|doct|odt|gdoc|txt|rtf|pdf|mht|htm|html|epub|djvu|xps|docm|dot|dotm|dotx|fodt|ott|fb2|xml))$/.exec(
              _config.document.fileType
            );
          if (!type) {
            window.alert(
            ...
            if (typeof type[1] === "string") _config.documentType = "cell";
            else if (typeof type[2] === "string")
              _config.documentType = "slide";
            else if (typeof type[3] === "string") _config.documentType = "word";
          }
        }
        ...
        if (
          _config.editorConfig.user &&
          _config.editorConfig.user.id &&
          typeof _config.editorConfig.user.id == "number"
        ) {
          _config.editorConfig.user.id =
            _config.editorConfig.user.id.toString();
          console.warn(
            'The "id" parameter for the editorConfig.user object must be a string.'
          );
        }
  ...
  function getAppPath(config) {
    ...
    if (typeof config.documentType === "string") {
      ...
      var type =
        /^(?:(xls|xlsx|ods|csv|xlst|xlsy|gsheet|xlsm|xlt|xltm|xltx|fods|ots)|(pps|ppsx|ppt|pptx|odp|pptt|ppty|gslides|pot|potm|potx|ppsm|pptm|fodp|otp))$/.exec(
          config.document.fileType
        );
      if (type) {
        if (typeof type[1] === "string") app = appMap["cell"];
        else if (typeof type[2] === "string") app = appMap["slide"];
      }
    }
    ...
    var userAgent = navigator.userAgent.toLowerCase(),
      ...
      isIE =
        !check(/opera/) && (check(/msie/) || check(/trident/) || check(/edge/)),
      isChrome = !isIE && check(/\bchrome\b/),
      isSafari_mobile =
        !isIE && !isChrome && check(/safari/) && navigator.maxTouchPoints > 0;

    path += app + "/";
    path +=
      config.type === "mobile" || isSafari_mobile
        ? "mobile"
    ...
    if (config.editorConfig) {
      ...
      if (
        ...
        (customization.toolbarNoTabs ||
          (config.editorConfig.targetApp !== "desktop" &&
            (customization.loaderName || customization.loaderLogo)))
      ) {
        index = "/index_loader.html";
      } else if (
        config.editorConfig.mode == "editdiagram" ||
        config.editorConfig.mode == "editmerge"
      )
  ...
  function getAppParameters(config) {
    ...
    if (config.editorConfig && config.editorConfig.targetApp !== "desktop") {
      if (
        ...
        if (config.editorConfig.customization.loaderName !== "none")
          params +=
            "&customer=" +
            encodeURIComponent(config.editorConfig.customization.loaderName);
      ...
      if (
        ...
        if (config.editorConfig.customization.loaderLogo !== "")
          params +=
            "&logo=" +
            encodeURIComponent(config.editorConfig.customization.loaderLogo);
      } else if (
        typeof config.editorConfig.customization == "object" &&
        ...
        if (
          config.type == "embedded" &&
          config.editorConfig.customization.logo.imageEmbedded
        )
          params +=
            "&headerlogo=" +
            encodeURIComponent(
          ...
          params +=
            "&headerlogo=" +
            encodeURIComponent(config.editorConfig.customization.logo.image);
      }
    }
    ...
    if (
      ...
      (config.document &&
        ...
        !config.document.permissions.review)
    )
      params += "&mode=view";
    ...
    if (
      ...
      !!config.editorConfig.customization.compactHeader
    )
      params += "&compact=true";
    ...
    if (
      config.editorConfig &&
      config.editorConfig.customization &&
      config.editorConfig.customization.toolbar === false
    )
  ...
  function createIframe(config) {
    ...
    iframe.setAttribute(
      "allow",
      "autoplay; camera; microphone; display-capture"
    );
